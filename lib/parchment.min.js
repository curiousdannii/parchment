 ...
 */
$.ajaxSetup({cache:true,dataType:"text",ifModified:location.protocol!=="file:"});var parchment={};(function(f){var j=/\(['"](.+)['"]\)/,g=$.extend,a=function(q,r){var r=r||[],p=0,o;for(o=q.length%8;p<o;++p){r.push(q.charCodeAt(p)&255)}for(o=q.length;p<o;){r.push(q.charCodeAt(p++)&255,q.charCodeAt(p++)&255,q.charCodeAt(p++)&255,q.charCodeAt(p++)&255,q.charCodeAt(p++)&255,q.charCodeAt(p++)&255,q.charCodeAt(p++)&255,q.charCodeAt(p++)&255)}return r},m=function(s,r){var r=r||"",q=0,o,p=String.fromCharCode;for(o=s.length%8;q<o;++q){r+=p(s[q])}for(o=s.length;q<o;){r+=(p(s[q++])+p(s[q++])+p(s[q++])+p(s[q++])+p(s[q++])+p(s[q++])+p(s[q++])+p(s[q++]))}return r};if(f.atob){var c=function(p,o){return a(atob(p),o)},i=function(p,o){return btoa(m(p,o))}}else{var h="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=",b=(function(){var o=[],p=0;for(;p<h.length;p++){o[h.charAt(p)]=p}return o})(),c=function(t,r){var r=r||[],u,q,p,y,x,w,v,s=0,o=t.length;while(s<o){y=b[t.charAt(s++)];x=b[t.charAt(s++)];w=b[t.charAt(s++)];v=b[t.charAt(s++)];u=(y<<2)+(x>>4);q=((x&15)<<4)+(w>>2);p=((w&3)<<6)+v;r.push(u,q,p)}if(v==64){r.pop()}if(w==64){r.pop()}return r},i=function(t,r){var r=r||"",u,q,p,y,x,w,v,s=0,o=t.length;while(s<o){u=t[s++];q=t[s++];p=t[s++];y=u>>2;x=((u&3)<<4)+(q>>4);w=((q&15)<<2)+(p>>6);v=p&63;r+=(h.charAt(y)+h.charAt(x)+h.charAt(w)+h.charAt(v))}if(isNaN(q)){r=r.slice(0,-2)+"=="}else{if(isNaN(p)){r=r.slice(0,-1)+"="}}return r}}var n=jQuery.ajaxSettings.xhr(),l={binary:n.overrideMimeType!==undefined,cross_origin:n.withCredentials!==undefined},d=function(q,u,r){var s=function(v){u(c(j.exec(v)[1]))},o=/:\/\//,t=/^file:\/\//,p={error:k,url:q};if(!l.binary&&(t.test(q)||t.test(location)&&!o.test(q))){throw new FatalError("Can't load local files with this browser, sorry!")}if(r||!l.binary||(!l.cross_origin&&o.test(q))){g(p,{data:{encode:"base64",url:q},success:function(v){u(c(v))},url:parchment.options.proxy_url})}if(l.binary){g(p,{beforeSend:e,success:function(w,v,x){if(w==""){d(q,u,true);return}u(a(w))}});if(p.data){p.data["encode"]="raw"}}if(q.slice(-3).toLowerCase()===".js"){p.success=s}$.ajax(p)},e=function(o){o.overrideMimeType("text/plain; charset=x-user-defined")},k=function(o,p){throw new FatalError("Error loading story: "+p)};n=null;f.file={text_to_array:a,array_to_text:m,base64_decode:c,base64_encode:i,download_to_array:d,support:l}})(window);(function(){window.gIsIphone=navigator.userAgent.match(/iPhone/i);var b;var a="0";$(document).ready(function(){b=document.getElementById("top-window");a="0";var d=navigator.appVersion.match(/MSIE (\d+)\./);if(d&&+d[1]<7){b.style.position="absolute";var c=function(){b.style.top=1*(document.documentElement.scrollTop+1*a)+"px"};window.onscroll=c;window.onresize=c}})})();(function(e){var g=IFF.extend({init:function a(o,j){this.title=j;if(o[0]<9){this.filetype="ok story naked zcode";this._super();this.chunks.push({type:"ZCOD",data:o});this.zcode=o}else{if(IFF.text_from(o,0)=="FORM"){this._super(o);if(this.type=="IFRS"){for(var m=0,h=this.chunks.length;m<h;m++){var n=this.chunks[m].type;if(n=="ZCOD"&&!this.zcode){this.zcode=this.chunks[m].data}else{if(n=="IFmd"){this.metadata=file.array_to_text(this.chunks[m].data);var k=$(this.metadata);if(k){if($("title",k)){this.title=$("title",k).text()}if($("ifid",k)){this.ifid=$("ifid",k).text()}if($("release",k)){this.release=$("release",k).text()}}}}}if(this.zcode){this.filetype="ok story blorbed zcode"}else{this.filetype="error: no zcode in blorb"}}else{if(this.type=="IFZS"){this.filetype="error: trying to load a Quetzal savefile"}else{this.filetype="error unknown iff"}}}else{this.filetype="error unknown general"}}},load:function b(h){if(this.zcode){h.loadStory(this.zcode)}}}),d=Class.extend({add:function(h){this[h.ifid]=h;if(h.url){this.url[h.url]=h}},url:{}}),c=function(i,h){var j,k=function(r){if($.isArray(r)){j=r}if(h.loaded_zmachine||e.GnustoEngine&&e.Quetzal&&e.EngineRunner&&e.Console&&e.WebZui&&j){h.loaded_zmachine=true;$("#progress-text").html("Starting interpreter...");var q=typeof console!==undefined?function(){}:function(s){e.console.log(s)},n=new GnustoEngine(q),l=new WebZui(n,q),p=new EngineRunner(n,l,q),o=new g(j.slice(),storyName);q("Story type: "+o.filetype);o.load(n);if(e.location.hash){var m=e.location.hash.slice(1);n.loadSavedGame(file.base64_decode(m));q("Loading savefile")}p.run()}};if(!h.loaded_zmachine){$.getScript("lib/gnusto.min.js",k);$.getScript("lib/zmachine.min.js",k)}file.download_to_array(i,k)},f=Class.extend({load:function(k){var h=new Querystring(),i=h.get("story",parchment.options.default_story);storyName=i.slice(i.lastIndexOf("/")+1);storyName=storyName?storyName+" - Parchment":"Parchment";e.document.title=storyName;if(this.stories.url[i]){var j=this.stories.url[i]}else{$("#progress-text").html("Retrieving story file...");c(i,this)}},stories:new d(),savefiles:{}});e.Library=f})(window);(function(a){var c=a.parchment;c.options={default_story:"stories/troll.z5.js",proxy_url:"http://2.latest.zcode.appspot.com/proxy/"};function b(){if(a.parchment_options){$.extend(c.options,parchment_options)}var d=new Library();c.library=d;d.load();if(location.href.slice(0,31)=="http://parchment.googlecode.com"){$.getScript("http://www.google-analytics.com/ga.js",function(){gat._getTracker("UA-7949545-1")._trackPageview()})}}$(b)})(window);