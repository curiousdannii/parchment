...
accept_specialinput(arg.specialinput);$(document).trigger({type:'GlkOutput',output:arg.content});jQuery.each(windowdic,function(winid,win){if(win.type=='buffer'&&win.needscroll){win.needscroll=false;if(!win.needspaging){var frameel=win.frameel;if(!perform_paging){frameel.scrollTop(frameel.get(0).scrollHeight);win.needspaging=false;}
...
$(document).trigger({type:'CharInput',input:{keyCode:charval}});gli_selectref.set_field(0,Const.evtype_CharInput);gli_selectref.set_field(1,win);gli_selectref.set_field(2,charval);gli_selectref.set_field(3,0);win.char_request=false;win.char_request_uni=false;win.input_generation=null;if(window.GiDispa)
GiDispa.prepare_resume(gli_selectref);gli_selectref=null;VM.resume();}
...
$(document).trigger({type:'LineInput',input:input});for(ix=0;ix<input.length;ix++)
