 ...
 */
(function(){var b=function(e,f){return e[f]<<24|e[f+1]<<16|e[f+2]<<8|e[f+3]},c=function(f,g){var e=String.fromCharCode;return e(f[g])+e(f[g+1])+e(f[g+2])+e(f[g+3])},d=Class.extend({init:function a(h){this.type="";this.chunks=[];if(h){if(c(h,0)!="FORM"){throw new Error("Not an IFF file")}this.type=c(h,8);var f=12,e=h.length;while(f<e){var g=c(h,f);var j=b(h,f+4);if(j<0||(j+f)>h.length){throw new Error("IFF: Chunk out of range")}this.chunks.push({type:g,offset:f,data:h.slice(f+8,f+8+j)});f+=8+j;if(j%2){f++}}}}});d.num_from=b;d.text_from=c;window.IFF=d})();
/*
