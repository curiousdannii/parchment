 ...
 */
(function(){var c=function(m,n){var n=n||[],k=0,j;for(j=m.length%16;k<j;++k){n.push(m.charCodeAt(k))}for(j=m.length;k<j;){n.push(m.charCodeAt(k++),m.charCodeAt(k++),m.charCodeAt(k++),m.charCodeAt(k++),m.charCodeAt(k++),m.charCodeAt(k++),m.charCodeAt(k++),m.charCodeAt(k++),m.charCodeAt(k++),m.charCodeAt(k++),m.charCodeAt(k++),m.charCodeAt(k++),m.charCodeAt(k++),m.charCodeAt(k++),m.charCodeAt(k++),m.charCodeAt(k++))}return n},g=function(o,n){var n=n||"",m=0,j,k=String.fromCharCode;for(j=o.length%8;m<j;++m){n+=k(o[m])}for(j=o.length;m<j;){n+=(k(o[m++])+k(o[m++])+k(o[m++])+k(o[m++])+k(o[m++])+k(o[m++])+k(o[m++])+k(o[m++]))}return n};if(atob){var d=function(k,j){var j=j||[];return c(atob(k),j)},f=function(k,j){var j=j||"";return btoa(g(k,j))}}else{var a="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=",e={A:0,B:1,C:2,D:3,E:4,F:5,G:6,H:7,I:8,J:9,K:10,L:11,M:12,N:13,O:14,P:15,Q:16,R:17,S:18,T:19,U:20,V:21,W:22,X:23,Y:24,Z:25,a:26,b:27,c:28,d:29,e:30,f:31,g:32,h:33,i:34,j:35,k:36,l:37,m:38,n:39,o:40,p:41,q:42,r:43,s:44,t:45,u:46,v:47,w:48,x:49,y:50,z:51,0:52,1:53,2:54,3:55,4:56,5:57,6:58,7:59,8:60,9:61,"+":62,"/":63,"=":64},d=function(q,m){var m=m||[],p,n,k,u,t,s,r;for(var o=0,j=q.length;o<j;){u=e[q.charAt(o++)];t=e[q.charAt(o++)];s=e[q.charAt(o++)];r=e[q.charAt(o++)];p=(u<<2)+(t>>4);n=((t&15)<<4)+(s>>2);k=((s&3)<<6)+r;m.push(p,n,k)}if(r==64){m.pop()}if(s==64){m.pop()}return m},f=function(q,m){var m=m||"",p,n,k,u,t,s,r;for(var o=0,j=q.length;o<j;){p=q[o++];n=q[o++];k=q[o++];u=p>>2;t=((p&3)<<4)+(n>>4);s=((n&15)<<2)+(k>>6);r=k&63;m+=(a.charAt(u)+a.charAt(t)+a.charAt(s)+a.charAt(r))}if(isNaN(n)){m=m.slice(0,-2)+"=="}else{if(isNaN(k)){m=m.slice(0,-1)+"="}}return m}}var h=IFF.extend({init:function i(r,m){this.title=m;if(r[0]<9){this.filetype="ok story naked zcode";this._super();this.chunks.push({type:"ZCOD",data:r});this.zcode=r}else{if(IFF.text_from(r,0)=="FORM"){this._super(r);if(this.type=="IFRS"){this.images=[];this.resources=[];for(var p=0,k=this.chunks.length;p<k;p++){var q=this.chunks[p].type;if(q=="RIdx"){for(var o=0,s=IFF.num_from(this.chunks[p].data,0);o<s;o++){this.resources.push({usage:IFF.text_from(this.chunks[p].data,4+o*12),number:IFF.num_from(this.chunks[p].data,8+o*12),start:IFF.num_from(this.chunks[p].data,12+o*12)})}}else{if(q=="ZCOD"&&!this.zcode){this.zcode=this.chunks[p].data}else{if(q=="IFmd"){this.metadata=g(this.chunks[p].data);var n=$(this.metadata);if(n){this.metadataDOM=n;if($("title",n)){this.title=$("title",n).text()}if($("ifid",n)){this.ifid=$("ifid",n).text()}}}else{if(q=="Fspc"){this.frontispiece=IFF.num_from(this.chunks[p].data,0)}}}}}if(this.zcode){this.filetype="ok story blorbed zcode"}else{this.filetype="error: no zcode in blorb"}}else{if(this.type=="IFZS"){this.filetype="error: trying to load a Quetzal savefile"}else{this.filetype="error unknown iff"}}}else{this.filetype="error unknown general"}}},load:function b(j){if(this.zcode){j.loadStory(this.zcode)}window.document.title=this.title+" - Parchment"}});window.file={text_to_array:c,array_to_text:g,base64_decode:d,base64_encode:f,story:h}})();